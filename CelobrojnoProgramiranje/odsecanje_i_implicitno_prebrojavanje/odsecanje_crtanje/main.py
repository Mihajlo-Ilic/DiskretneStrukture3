...
if __name__ == "__main__":
    ...
    dot = Graph(comment='Odsecanje i ogranicavanje',format='png')
    dot.attr(nodesep='1')
    ...
    while len(red)>0:
        ...
        if len(cvorovi)>0:
            ivice.append([str(cvor_index),str(t[1]),t[2]])
        ...
        if tab!=None:
            ...
            cvorovi.append([str(cvor_index),0,tip+' f = {}'.format(round(f,2))+'\n{}'.format([round(j,2) for j in x[:br]])])
            ...
            if not all(i.is_integer() for i in x[:br]):
                ...
                red.append([levi_prob,cvor_index,'x{}<={}'.format(ind,int(levo))])
                red.append([desni_prob,cvor_index,'x{}>={}'.format(ind,int(desno))])

            else:
                cvorovi.append([str(cvor_index),2,tip+' f = {}'.format(round(f,2))+'\n{}'.format([round(j,2) for j in x[:br]])])
            ...
            cvorovi.append([str(cvor_index),-1,'Nema resenje'])
    ...
    for n in cvorovi:
        if n[1] == -1:
            dot.node(n[0],n[2],style='filled,bold',fillcolor='#8c230f',fontcolor='white',shape='box')
        if n[1] == 2:
            dot.node(n[0],n[2],style='filled,bold',fillcolor='#258d99',fontcolor='white',shape='box')
        if n[1] == 3:
            dot.node(n[0],n[2],style='filled,bold',fillcolor='#146321',fontcolor='white',shape='box')
        if n[1] == 0:
            dot.node(n[0],n[2])


    for e in ivice:
        dot.edge(e[1],e[0],e[2])

    dot.render('resenje_'+sys.argv[1])