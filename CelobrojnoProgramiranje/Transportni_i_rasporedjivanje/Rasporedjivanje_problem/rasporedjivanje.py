...
def ucitaj_problem():
    ...
    for line in lines:
        ...
        C.append([float(i) for i in values])

    #ima b jedinica koliko i kolona u jednom c redu
    b = [1 for i in C[0]]
    #ima a jedinica koliko i redova
    a = [1 for i in C]

    return a,b,C

def standardizuj_problem(a,b,C):
    #ovde proveravamo da li je duzina a = duzina b
    ...
    if len(a) > len(b):
        if pisi_korake:
            print('nema isti broj ljudi kao poslova dodajemo fiktivne poslove')
        n = abs(len(a)-len(b))

        for i in range(n):
            for red in C:
                red.append(max_el+100)
            for red in Q:
                red.append(-math.inf)
            for red in V:
                red.append(1)
        ...
        if pisi_korake:
            print('nema isti broj ljudi kao poslova dodajemo fiktivne ljude')
        n = abs(len(a)-len(b))
        for i in range(n):
            C.append([max_el+100 for j in C[0]])
...
def korak4(a,b,C,Q,i,j):
    ...
    #tj ako smo isli kroz jedan red uzimamo samo 2 najudaljenije tacke isto tako za kolonu
    za_brisanje = []
